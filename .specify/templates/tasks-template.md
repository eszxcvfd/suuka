...
**Tests**: Include test tasks for all behavior changes. Tests may be omitted only for non-behavioral updates (for example documentation-only changes) with explicit rationale.
...
- [ ] T004 [P] Configure automated test execution in CI/local workflow
- [ ] T005 [P] Define baseline performance and UX validation approach
...
- [ ] TXXX Run UX consistency audit across changed user journeys
- [ ] TXXX Validate performance budgets and capture evidence
...
- Verify behavior-change tests fail before implementing
